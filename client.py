...
def c(col): return "\33[0m" if col == 0 else (
    "\33[3"+str(col)[0]+";4"+str(col)[1]+"m")


class ContextManager:
    ''' Hello we are pictonary'''
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if (exc_type != None) and (self.debugLevel >= self.dangerLevel):
            ...
            with open(__file__, "r") as src:
                print("  "+dedent(src.readlines()
                      [exc_tb.tb_lineno-1].rstrip()))
...
class GameScreen:
    def __init__(self, playerName, servPort, servIP):
        ...
        self.currentMenu.add_command(
            label="New word - You paint!", command=self.newWord)
        self.currentMenu.add_command(
            label="Change name", command=self.changeName)
        self.currentMenu.add_command(label="Cheat", command=self.cheat)
        self.gamemenu.add_command(label="Quit", command=self.onExit)

        # Main window:
        self.window.title('pictionary.py')
        self.window.geometry('{}x{}'.format(810, 617))
        self.top_frame = Frame(self.window, bg='grey',
                               width=800, height=50, pady=3)
        self.center = Frame(self.window, bg='black',
                            width=800, height=40, padx=0, pady=3)
        self.btm_frame = Frame(self.window, bg='black',
                               width=800, height=45, pady=0)
        ...
        self.ctr_left = Frame(self.center, bg='grey',
                              width=500, height=500, padx=0, pady=0)
        self.ctr_right = Frame(self.center, bg='black',
                               width=300, height=300, padx=3, pady=0)
        self.ctr_left.grid(row=0, column=0, sticky="ns")
        self.ctr_right.grid(row=0, column=2, sticky="ns")
        self.ctr_r_top = Frame(self.ctr_right, bg='grey', width=300,
                               height=188, padx=0, pady=0, borderwidth=2, relief="groove")
        self.ctr_r_btm = Frame(self.ctr_right, bg='grey', width=300,
                               height=318, padx=0, pady=0, borderwidth=2, relief="groove")
        ...
        self.btm_left = Frame(self.btm_frame, bg='grey',
                              width=508, height=50, padx=0, pady=0)
        self.btm_right = Frame(self.btm_frame, bg='black',
                               width=306, height=50, padx=0, pady=0)
        ...
        self.upperBox = Text(self.ctr_r_top, bg='black', width=41, height=12)
        self.textbox = Text(self.ctr_r_btm, bg='black', width=41, height=21)
        self.upperBox.grid(row=0, column=0)
        self.textbox.grid(row=0, column=0)
        self.entrybox = Text(self.btm_right, bg='black',
                             width=41, height=3, borderwidth=1, relief="sunken")
        self.entrybox.grid(row=2, column=0)
        self.entrybox.bind("<Return>", self.pressedEnter)
        self.clearButton = Button(
            self.btm_left, text='Clear', command=self.eraseCanvas)
        self.clearButton.grid(row=0, column=0)

        ######## Just for dev testing #########
        self.paintModeToggle = IntVar()
        self.paintModeBtn = Checkbutton(
            self.btm_left, text="Debug Paint mode", variable=self.paintModeToggle)
    ...
    def msgRouter(self):
        ...
        if self.msgType == "NW":
            ...
            if self.msgName == self.playerName:
                ...
                with calm:
                    self.inTheBox = str(
                        self.msgName + ", it's your time to paint!\nThe word is:\n" + self.msgValue)
                with calm:
                    self.textbox.insert(END, self.inTheBox)
                with calm:
                    self.textbox.see("end")
    ...
    def paint(self, event):
        ...
        if self.paintModeOn:
            if self.old_x and self.old_y:
                self.c.create_line(self.old_x, self.old_y, event.x,
                                   event.y, width=4, fill=self.paintColor)
            self.sendMsgValue = [self.old_x, self.old_y,
                                 event.x, event.y, self.paintColor]
            self.old_x = event.x
            self.old_y = event.y
    ...
    def recreatePaint(self):
        try:
            ...
            self.debugInfo(
                dangerLevel=2, exceptContext="recreatePaint, ast.literal_eval(self.coords)")
        try:
            self.x1, self.y1, self.x2, self.y2, self.paintColor = self.coords[
                0], self.coords[1], self.coords[2], self.coords[3], self.coords[4]
        except:
            self.debugInfo(
                dangerLevel=2, exceptContext="recreatePaint, self.x1... = coords[0]...")
        if self.paintColor != "clear":
            try:
                self.c.create_line(self.x1, self.y1, self.x2, self.y2,
                                   width=4, fill=self.paintColor)  # , tags="drawing"
            except TclError:
                pass
    ...
    def eraseCanvas(self):
        # If the player is currently painting, this clears everyone else's canvas.
        if self.paintModeOn:
            self.c.delete("all")  # Clear the canvas
            # I didn't want to over-complicate things, so the
            self.sendMsgValue = [0, 0, 0, 0, "clear"]
            # screen-clearing command pretends to be a color.
            # Telling the server it's drawing stuff with "CO"
            self.sendMsg = ("CO//"+str(self.sendMsgValue))
            self.sendMsgNow()
...
class ShowStartScreen():
    def __init__(self):
        ...
        self.joinBtn = Button(
            self.startScreen, text='Join Game', command=self.onJoin)
    ...
    def onJoin(self):  # Reminder to implement a config file system!
        ...
        newGameScr = GameScreen(
            servIP=self.servIP, servPort=self.servPort, playerName=self.playerName)
